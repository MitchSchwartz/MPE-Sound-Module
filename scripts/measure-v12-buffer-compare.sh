#!/bin/bash
# V12 — compare 1024×2 vs 512×2 xrun rate at Cloud Horn @5 (governor on).
#
...
if [ "$PILOT" -eq 1 ]; then
    MINUTES=2
    echo "V12 pilot: ${MINUTES} min single arm @ 1024×2"
    mkdir -p "$OUTPUT_DIR"
    exec "$SCRIPT_DIR/measure-soak-instrument.sh" \
        --minutes "$MINUTES" --buffer 1024 --periods 2 \
        --governor on --patch-name "$PATCH_NAME" --voices "$VOICES" \
...

{
    ...
    echo "minutes_per_arm=${MINUTES} order=${ORDER} first=${FIRST} patch=${PATCH_NAME} voices=${VOICES}"
