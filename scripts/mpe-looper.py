...
def main(argv: list[str] | None = None) -> int:
    ...
    if looper_guard_blocked(
        engine=os.environ.get("MPE_AUDIO_ENGINE"),
        looper_enabled=os.environ.get("MPE_LOOPER_ENABLED", "0"),
    ):
        print(LOOPER_GUARD_MESSAGE, file=sys.stderr, flush=True)
        if os.environ.get("MPE_LOOPER_SERVICE") == "1" or os.environ.get("INVOCATION_ID"):
            print("LOOPER-GUARDED", flush=True)
        return looper_guard_exit_code(
            looper_service=os.environ.get("MPE_LOOPER_SERVICE") == "1",
