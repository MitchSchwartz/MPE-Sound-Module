#!/usr/bin/env bash
# Build native/mpe-live-monitor when libjack-dev is present.
...
DIR="$ROOT/native/mpe-live-monitor"
...
if ! pkg-config --exists jack 2>/dev/null; then
    ...
    echo "SKIP: libjack-dev not installed — mpe-live-monitor not built" >&2
    exit 0
fi

make -C "$DIR" clean all
echo "Built $DIR/mpe-live-monitor"
