...
| O3 | **V12** — buffer compare | **Preflight gate cleared 2026-08-23 18:11** @ Pi `28a220b`: 45s spot **33.20%** ≥ **28%** floor (`~/v12-preflight-post28-20260823-181123.log`). **Short soak 10 min** @512 governor off: **0** xruns, per-minute plausibility **10/10** @ ~33% (`~/v12-short-20260823-181337.log`). Full **30 min two-arm** compare still **not** re-run — prior VOID run + no 512↔1024 citation until both arms pass. See `V12-RESULT-2026-08-23.md` §Post-28% preflight gate. | preflight done; full V12 pending |
...
**O4, O6 and O1b are closed.** V12 canonical load **~33%** reconciled; reference **~58%** not restored. **Preflight @28% passed** on Pi `28a220b`; **10 min** single-arm soak certifies stable loaded hold @512. **Next:** full `measure-v12-buffer-compare.sh` (both arms, per-minute plausibility) or A4 pass 2; optional Mitch gate for **hw:2** cell. O1 (P7 baseline) and O5 unchanged.
