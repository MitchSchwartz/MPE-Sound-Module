...
- **Vol fader vs DAC:** on a card with one unambiguous hardware volume the touch **Vol** fader *is* the DAC level (post-graph, so this meter does not move with it); otherwise it trims Surge `amp/volume`. See `patch_browser/device_volume.py`.
