...
| **Recording** | Stop recording early (optional v1 — else wait until loop length full). |
| **Has clip, stopped** | Launch clip (start playback). |
| **Has clip, playing** | Stop clip at **next bar boundary** (same quantize grid as scene stop). |
