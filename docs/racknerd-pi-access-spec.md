...
2. ✅ **systemd units versioned** (2026-08-16). All in [`config/`](../config/) as templates, installed by [`scripts/install-units.sh`](../scripts/install-units.sh), which reproduces **recorded enable state** (10 enabled, 3 deliberately disabled, 1 static) rather than blanket-enabling — the appliance's boot behaviour is not uniform. `--diff` run against the live appliance reports **no drift**; the rendered templates are byte-identical to what is running. *(Updated 2026-08-17: the pre-rendered `systemd/` copy was collapsed into `config/` — two committed copies of every unit meant adding to one silently missed the other.)* Secret-scanned clean.
