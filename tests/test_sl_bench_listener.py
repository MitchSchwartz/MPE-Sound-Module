...
class SlBenchStateListenerTests(unittest.TestCase):
    def test_register_all_loops(self) -> None:
        by_loop = {
            0: TrackGesture(loop=0, hold_ms=1000.0, debounce_ms=0.0),
            1: TrackGesture(loop=1, hold_ms=1000.0, debounce_ms=0.0),
        }
        session = MagicMock()
    ...
    def test_on_update_routes_to_gesture(self) -> None:
        fs = TrackGesture(loop=3, hold_ms=1000.0, debounce_ms=0.0)
    ...
    def test_on_update_routes_loop_len_to_surface_without_gesture(self) -> None:
        surface = MagicMock()
